...
- `loop.js` — `SyncGame` class: round flow (5 rounds, no roles), my+peer composition state, lock state, set-overlap scoring, history. Uses `pickPrompt(roomCode, round)` for deterministic per-round prompt.
...
- `content.js` — the deck of 100+ absurdly specific feelings, the 60-emoji palette, `pickPrompt`, `overlapScore` (the set-intersection scorer), `compositionToSet` (multi-codepoint-safe palette tokenizer), and `VERDICTS` percentile bands.
...
- The PALETTE includes multi-codepoint graphemes (😵‍💫, 👁️, 🛣️, etc.). Use `compositionToSet` from `content.js` to tokenise wire-format strings — never split emoji strings on JS string indexing or characters will fragment.
