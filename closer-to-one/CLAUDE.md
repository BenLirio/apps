...
- **`rank` / `rankTotal` are the leaderboard-style readout** ("#42 of 1,234") computed from the same histogram. `rank = 1 + count of session-averages with strictly smaller |delta|` (perfect ties don't share a rank — the player who submitted earliest holds the lower number). Both fields are `null` until bins load. Display alongside `beatPct` rather than instead of — they answer different questions ("how many people did better?" vs. "what fraction did better?").
